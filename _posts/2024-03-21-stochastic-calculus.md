...
2. $$Cov(B(s),B(t)) = E(B(s)B(t)) = \min(s,t)$$.
  - 证明: $$t > s > 0$$时有$$Cov(B(s),B(t)) = E(B(s) - B(0))E(B(t) - B(s)) + E(B^2(s)) = s$$.
3. 显然, Brownian motion是一个martingale.
4. $$B(t)^2 - t$$也是一个martingale.
  - 证明: 对于任意的$$t, s > 0$$, 有$$E(B^2(t + s) - (t + s) \vert \mathcal{F_t}) = B^2(t) + 2E(B(t)(B(t + s) - B(t)) \vert \mathcal{F_t}) + E((B(t + s) - B(t))^2 \vert \mathcal{F_t}) - (t + s)= B^2(t) - t$$
