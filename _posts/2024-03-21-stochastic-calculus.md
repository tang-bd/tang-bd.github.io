...
2. $$ E(\int_a^b G(t)dB(t)) = 0 $$.
  - 证明思路: 由于$$F(t_i)$$与$$B(t_{i+1}) - B(t_i)$$独立, $$\begin{split*} E(S_n) = \sum_{i=0}^{n-1} E(G(t_i^n)(B(t_{i+1}^n) - B(t_i^n))) \\ = \sum_{i=0}^{n-1} E(G(t_i^n))E(B(t_{i+1}^n) - B(t_i^n)) = 0 \end{split*}$$. 则可进一步证明Itô integral的期望为$$0$$.
